...
SoftwareSerial serialcom(D1,D2);//Tx Uno,Rx,Uno
...
void label(){
  ...
  Blynk.virtualWrite(V2,bin_lvl); // updates the VirtualPin V1 
}
void check(){
  label();
  alert();
...
void alert(){
  if( bin_lvl>"90"){
    Blynk.virtualWrite(V4,1);//trigger the virtul pin V0 if bin level is greater than 90
    
  }
  else{
    Blynk.virtualWrite(V4,0);//if bin level is less than 90 then virtual pin set to 0
    }
}
